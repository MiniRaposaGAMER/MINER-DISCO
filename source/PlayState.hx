...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['at least i exist', 0.69], //From 60% to 68%
		['NICE', 0.7], //69%
		['TOP', 0.8], //From 70% to 79%
		['GOOD', 0.9], //From 80% to 89%
		['Incredible', 1], //From 90% to 99%
		['HEROBRINE!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
					...
					case 'toastie':
						if(/*ClientPrefs.framerate <= 60 &&*/ ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing) {
							unlock = true;
										...
										case 'minecrafter':
						if(Paths.formatToSongPath(SONG.song) == 'crossover-no-mine' && !usedPractice) {
							unlock = true;
						}
